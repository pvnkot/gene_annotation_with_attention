...
def train(train_inputs, train_labels, test_data, test_labels, sent_size, device):
    fcNet = net.Attention_Net(sent_size, device)
    if torch.cuda.is_available():
        fcNet = fcNet.cuda(device=device)
...
def train_epoch(model, inputs, labels, optimizer, criterion, device):
    model.train()
    losses = []
    j = 0
    correct, wrong = 0,0
    ...
    for i in range(0, len(inputs), Config.batch_size):
        ...
        data_batch = autograd.Variable(data_batch).to(device=device)
        labels_batch = autograd.Variable(labels_batch).to(device=device)
        optimizer.zero_grad()
...
def train_model(train_inputs, train_labels, test_data, test_labels, model, optimizer, criterion, sent_size, with_attention, device):
    losses = []
    ...
    for epoch in range(Config.num_epochs):  # loop over the dataset multiple times
        loss, acc = train_epoch(model, train_inputs, train_labels, optimizer, criterion, device)
        losses.append(loss)
        train_accuracy = 100*(acc[0]/(acc[0]+acc[1]))
        train_accuracies.append(train_accuracy)
        torch.save(model.state_dict(), Config.test_model_name)
        test_accuracy = test.test(test_data, test_labels, sent_size, Config.test_model_name, device)
